...
export default function PostAside(props:Props) {
    const {postId} = props
  return (
    <aside>
      <div className="font-system grid grid-rows-2 md:grid-rows-1 md:grid-cols-2 shadow-form mb-7 rounded-md">
        <div className="bg-bg-secondary p-10 rounded-t-md md:rounded-tr-none md:rounded-l-md">
          <h4 className="font-bold text-xl mb-5">
            Learn to Build Resilient React Components
          </h4>
          <p className="mb-7">
            Get a one week email course and learn how I think about writing
            React components based on 4 Principles.
          </p>
          <div>
            <svg
        ...
        <form className="bg-transparent p-10 rounded-b-md md:rounded-bl-none md:rounded-r-md">
          <input
            placeholder="Your first name"
            className="mb-4 p-3 rounded placeholder:text-[#848585] placeholder:opacity-50 w-full"
          />
          <input
            placeholder="Your email address"
            className="mb-4 p-3 rounded placeholder:text-[#848585] placeholder:opacity-50 w-full"
          />
          <button className="bg-[#d54474] rounded-3xl h-[50px] w-[150px] font-bold">
            Start Learning
          </button>
      ...
      <h3 className="mb-7">
        <Link
          href="/"
          className="font-montserrat font-black text-2xl text-textLink"
        >
          Overreacted
        </Link>
      </h3>
      ...
      <nav className="mt-14">
        <ul className="flex items-center justify-between">
          <li>
            <Link href={`/${postId - 1}`} className="link">
              ← Previous
            </Link>
          </li>
          <li>
